...
export const defaultDetectScript = () => fileURLToPath(new URL('../skills/rdk-device-skills/rdk-diagnostic/scripts/detect_rdk.sh', import.meta.url));
/** Parse the `KEY=value` lines printed by detect_rdk.sh. Exported for tests. */
export function parseDetectOutput(output) {
    const fields = {};
...
export async function runDeviceDetect(customScript, timeoutMs = 30_000) {
    const script = customScript ?? defaultDetectScript();
    const result = { detected: false, code: null, script };
    try {
...
//# sourceMappingURL=device-detect.js.map