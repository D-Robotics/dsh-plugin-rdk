...
async function acquireRepo(info, source) {
    if (!isGitUrl(source)) {
        if (existsSync(join(source, 'setup.sh')))
            return { dir: source, used: source };
 ...
 */
export async function runOeSetup(input) {
    const info = OE_PACKS[input.pack];
    if (info === undefined) {
        return {
            ok: false,
            pack: input.pack,
    ...
    try {
        ...
        return {
            ...
            exitCode: null,
            output: [],
            reason: `failed to acquire pack repository: ${error.message}`,
        };
    }
...
//# sourceMappingURL=oe-setup.js.map